...
const { scanTargets } = require('./scanLogic');
...
const CONFIG = {
  ...
  MIN_SCORE: 58,           // 表示する最低スコア
  DISPLAY_LIMIT: 48,       // 最大表示件数
};
// ====================================================
...
async function doScan() {
  try {
    ...
    // 画面が期待する形 { symbol, name, price, change } に変換
    const symbols = results
      .filter((r) => r.kind === 'stock')
      .filter((r) => r.direction !== 'fail' && r.direction !== 'hidden')
      .filter((r) => typeof r.price === 'number' && r.price <= CONFIG.PRICE_LIMIT)
      .filter((r) => r.score >= CONFIG.MIN_SCORE)
      .sort((a, b) => b.score - a.score)
      .slice(0, CONFIG.DISPLAY_LIMIT)
      .map((r) => ({
        symbol: r.code,
        name: r.name,
        price: r.price,
